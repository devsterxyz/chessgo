export const INIT_GAME = "init_game"
export const CANCEL_MATCHMAKING = "cancel_matchmaking"
export const GAME_STARTED = "game_started"
export const MOVE = "move"
export const RESUME_GAME = "resume_game"
export const GET_ACTIVE_GAME = "get_active_game"
export const RESIGN_GAME = "resign_game"
export const DRAW_OFFER = "draw_offer"
export const DRAW_ACCEPT = "draw_accept"
export const DRAW_DECLINE = "draw_decline"
export const REMATCH_REQUEST = "rematch_request"
export const REMATCH_CANCEL = "rematch_cancel"
export const REMATCH_ACCEPT = "rematch_accept"
export const REMATCH_DECLINE = "rematch_decline"
export const LEAVE_FINISHED_GAME = "leave_finished_game"
